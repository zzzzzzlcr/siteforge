...
- **截图默认只在「没做成」和「click/goto 且 `progress=false`」的步骤落**。
  `--shots all` 时每步都落。
  > 2026-09-16 按 Task 3 的实际实现收窄：字面「只在 `progress=false` 落」会让**每个填表步都留图**
  > —— 而 `diff` **判不了「填进去了没有」**（R20：`PageModel` 不带字段值），
  > 于是 fill 步的 `progress` 恒 false、图恒落。按括号里的**意图**实现：失败步一定留图，填/选步不留。
