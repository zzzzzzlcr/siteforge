...
| **D15** | **产品定位：运营能自维护的自动化平台，不是 AI 脚本生成器** | 用户 2026-09-16：「本质项目是要**剥离技术人员**，让运营也可以参数脚本制作和脚本修复」。⇒ 判据从「生成的脚本对不对」变成「**运营能不能自助**」；**Build 是 Debug 的副产品**（修好一个站时路径本身就记录了；Build = 没有旧脚本时走一遍同样的探索+记录，共用同一套 trace/patch 机制） |
