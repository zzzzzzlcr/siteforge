...
## 最后一次复扫：**ready to merge**（7 项全 ADDRESSED，无断言削弱、无越界、无当前破坏）

Ruling C84（**一个假警报，记下**）：复扫方报「review 包被截断在 800 行」。
  **实测证伪**：脚本是纯 `git diff -U10 > $out` 无截断逻辑；重新生成同范围得 793 行、
  **尾行逐字相同**。那是 **`-U10` 的 10 行上下文正好用完** —— hunk 停在那儿是正常的。
  ⇒ **今天所有审查的 diff 基线是完整的。**
  这条本身是当日主题的**反向**实例：把一个正常现象（上下文窗口耗尽）读成了故障（文件被截断）。
Ruling C85（**park 三条潜在假红，附裁决**）：复扫指出新加的不变量有三处**潜在假红**
  （都不被当前夹具触发，且**是响的、不是静默的**）：
  · `bbox[1]=Math.round(r.top)` 与 `above_fold: r.top<innerHeight` **舍入口径不一致** ——
    `r.top ∈ [vh-0.5, vh)` 时 JS 判 true 而 Go 侧算出 false → **正确树上假红**
  · `RelativeSize <= 0` 把舍入伪影（小于中位面积 ~5% 者恰为 0.0）编码成绑定失败
  · `x<0||y<0` 只对**未滚动**页面成立
  裁决：**park，交计划二**。理由：三条都只在「元素正好跨折线」这类边界上触发，
  且失败是**响的**（测试红），不是静默错判 —— 与「漏报真实盲区」不同量级。
  代价若错：第一条跨折线的夹具会给出一条令人困惑的红（花几分钟排查）。
Ruling C86（park 文档措辞）：新不变量的注释称其「环境无关」，复扫指出**略微过头**
  （它对舍入敏感）；另 `tools/cdp/README.md` 仍写「支持 CDP_PORT 环境变量」而未提优先级陷阱。
  裁决：**park**，与 C81 一并归计划二。
