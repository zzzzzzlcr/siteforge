...
rounds[0].done         = None                         # 第一道闸的「刚做完的」是空的
```

那四条写的是**脚下这一道闸**（`rounds[n-1]`）那一轮。历史轮次按 `visits` 落位 ——
**落位公式只有一处**（`_node_index`，下面那句「从末尾对齐」就是它），这一屏只复述：
第 k 轮那个节点 = `visits[len(visits) - (total - k) - landed + offset]`
（`offset`：正要做的取 0、刚做完的取 −1；`landed` 见下）。
⚠️ **别在这儿另写一个算式** —— 这里原先写的是「正要做的 = `visits[k-1]`；刚做完的 =
`visits[k-2]`」，**两条都差一格**（2026-09-20 复审逐字拿它当判据跑过，`k = 1..5` 全不符）。
**差的那一格是 `intake`**：`visits[0]` 是它，而它**不设闸、没有对应的轮** ——
所以「第 k 轮」永远比「第 k 个进过的节点」**晚一格**。在**对齐得上**的形状里
（`visits` 的长度正好是 `total` 或 `total + 1`，见 `_landed`）公式就是
**第 k 轮正要做的 = `visits[k]`**、**刚才做完的 = `visits[k-1]`**（下标从 0 起；
⚠️ **第 1 轮那一格是例外**：`done` 整个是 `None`（不是 `visits[0]`），理由见下面那段）。
拿「5 道闸、跑完」那种形状（`visits` = `intake, explore, draft, lint, selftest, deliver`）
逐字对一遍，四个数都在这儿：第 1 轮 `step` = `visits[1]` = `explore`；
第 2 轮 `step` = `visits[2]` = `draft`、`done.step` = `visits[1]` = `explore`；
第 5 轮 `step` = `visits[5]` = `deliver`、`done.step` = `visits[4]` = `selftest`。
（⚠️ 站在闸上等人时，**脚下这一轮**那个节点**还没进 `visits`**（正是它在等你的那一步）
⇒ `_node_index` 给不出下标，那一格的 `step` 从**闸上**抄，不是从这个公式来。
「对最后一轮 `visits[-1]` 就是它」**只在脚下那一轮**成立，且成立的是 `step` 那一格。）
**不这么展开的话，每张卡都会说同一个节点**，那是编话：站在第 2 道闸上，第 1 轮那张卡说的
必须是**它前面那道闸**（2026-09-20 起是 `explore`；在那之前第一道闸是 `intake`）。

⚠️ **`rounds[0].done = None` 那句的理由改过两次，现在这一版才是准的**：
原先写「第一道闸之前**什么都没跑**」—— `intake` 不设闸之后这句话**是假的**（第一道闸之前
`intake` 跑过：它校验了开场白、修站那条路还读了底稿）。第二版换成「这条卡这一格要的东西
它**一样都没有**」—— 也**说满了**（2026-09-20 复审逐条量过，我在这儿更正）：
`shots` 那一格缺的**只是 `before` 那一半**（`after` 就是 `pause-1`，**存在**），
`steps` 那一格也**有话说**（`_steps_of` 走 `done_step != EXPLORE` 那一支，回的是
`NOT_EXPLORE_SAY` 那句解释），空的是那张清单本身。
**真的理由是配对的形状**（`_card` 里那句 `if k > 1`）：`done` 要配一对闸拍
`pause-<k-1>` / `pause-<k>`，而第 1 轮要的那一对是 `pause-0` / `pause-1` ——
**`pause-0` 不存在**（清单从第一道闸才开始拍）⇒ 这一格**在这张卡的 schema 里本来就
放不进去**，与 `intake`「跑没跑过」无关。
⇒ **空着是「这一格在这张卡的形状里没有对应的东西」，不是「没跑过」**。这条不许读成后者。
（`visits` 里它照样在 —— 那张卡的「正要做的」正是 `explore`，而它要确认的那份开场白
摊在**同一张卡的 `facts`** 上，见 `graph._brief_facts` 与 `_card` 里那一格。）
...
def project(values: dict, gate: Optional[dict], *, job_id: str, status: str, say: str,
    ...
    参数（全是**输入** —— 这个函数不读任何别的东西）：
      ...
      - `gate`：`Service._project` **算好的**那道闸（`/job/{id}` 用的同一份）。
        ⚠️ **它上面那两格（`ask` / `facts`）都要落到卡片上**：`ask` 进 `card["say"]`、
        `facts` 进 `card["facts"]`（都只在**脚下这一轮**）。少落一格 = 那一样东西
        到不了**运营那一屏** —— 复审 2026-09-20 §4.2 量的就是这个形状：facts 只活在
        载荷里、页面一格都不读，于是「成功判据」从屏幕上消失了；
...
def _card(k: int, *, total: int, shots: list, visits: list, journey, report, gate, stage: str,
    ...
    card = {
        ...
        #: 闸口摊开的**原始事实**（D11：给感知不给判断）—— 与 `say` 同一条规矩：
        #: 只有脚下这一道闸有（历史轮次的那一份早就不在 state 里了 ⇒ `{}`，**不许编**）。
        #: ⚠️ 它**不是**可有可无的装饰，`rounds[0]` 上它尤其承重：`intake` 不设闸之后，
        #: 第一道闸（`explore`）摊的就是运营点「继续」之前要确认的那份**开场白**
        #: （`graph._brief_facts`：站点 / 目标 / **成功判据** / 模式 / 窗口 / 允许跳过的扰动 /
        #: 还缺的旋钮），而这**是屏幕上唯一还能看见「成功判据」的地方**。
        #: 复审 2026-09-20 §4.2 的现场：这份 facts 只活在 `/job/{id}` 与 `/live` 的
        #: `gate` 那两格载荷里，而那一屏**一格都不读** ⇒ 运营看不到它 —— 那不叫
        #: 「少问一次」，叫「把确认变成走过场」。
        #: ⇒ 它是**投影的一部分**（走 `rounds.project` 这一跳），不是接线信息。
        "facts": dict((gate or {}).get("facts") or {}) if here else {},
