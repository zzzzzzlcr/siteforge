...
同一个节点**，那是编话：站在第 2 道闸上，第 1 轮那张卡说的必须是**它前面那道闸**
（2026-09-20 起是 `explore`；在那之前第一道闸是 `intake`）。

⚠️ **`rounds[0].done = None` 那句的理由改过**（2026-09-20）：原先写的是「第一道闸之前
**什么都没跑**」—— `intake` 不设闸之后这句话**是假的**（第一道闸之前 `intake` 跑过：
它校验了开场白、修站那条路还读了底稿）。它**照样是空的**，但理由换成了**这条卡这一格
要的东西它一样都没有**：`done` 那一格要配 `shots`（夹住它的那一对闸拍）与 `steps`
（它自己的步子清单），而 `intake` 既不拍闸拍、也没有步子清单、更不是运营要回话的那一轮。
⇒ **空着是「没有可显示的」，不是「没跑过」**。这条不许读成后者。
（`visits` 里它照样在 —— 那张卡的「正要做的」正是 `explore`，而它要确认的那份开场白
摊在同一张卡的 `facts` 上，见 `graph._brief_facts`。）
