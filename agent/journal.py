...
⚠️ 落盘的**只能是 step dict**：原始工具返回**整个**不许塞进来（那里面有几百 KB 的 base64
截图，`browser_agent._summarize` 就是为这件事存在的）。这里不做检查（那是 `_summarize` 的职责，
两处都写就成了两份判据）——写在这里，是为了让下一个人知道**为什么**账本里看不见原始返回。

⚠️ **`2026-09-18 起有一个例外，而且是故意的**：step 里多了一格 `receipt`（契约 §二第 3 格，
CDP 回执**原文**）—— 它由 `browser_agent._receipt_of` 转抄，**在 `RECEIPT_MAX_CHARS` 处封顶**，
超了那一格自己写着「原文有多少字、这里留了多少」（标注，不是静默截断）。
所以「账本里不出现原始返回」这句话今天要读成：**不出现整份原始返回**；
动作类回执（`{"ok": true, "note": …}` 或那句报错）本来就几个字节，逐字在那儿。
