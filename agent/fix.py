...
**人给了「步骤表」就照那张表发号施令**（正文里带 `【步骤表（人给的，照它走）】` 那一段；
一行一步，通常写着「动作 + 选择器 ｜ 等多久 ｜ 出现哪个元素才算这一步成了」）：
那张表**就是规格** —— 别自己重新设计流程，也别把顺序改了。照它写的每一条：
- **动手前先等这一步要的那个元素**（可见）—— 等到才动手；没等到**重试**（最多 3 次）。
- 动作之后 **等「下一步的关键元素」出现**：人给了等待区间就**用他的区间**（例如「30-50 秒」
  ⇒ 在区间内等/轮询，**别**自己拍一个固定值、更**别**不等就往下走）；没给区间就自己定一个
  合理的（几秒起）+ 重试。
- **出现了才算这一步成了**，接着做下一步；**没出现 ⇒ 这一步没成**：重做这一步（≤3 次），
  还不行就**如实报失败**（写清第几步、等的哪个元素、等了多少秒、重试了几次）。
