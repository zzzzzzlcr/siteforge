...
def has_script_say(value: Any) -> str:
    ...
    |---|---|---|
    | `True` | 有脚本或配置 | 不用管 |
    | `False` | 有配置，可是里面是空的 | **去补内容**（这份配置没东西可跑） |
    | `None` | 没有配置，谈不上 | **去建配置**（连配置都没有） |

    ⚠️ **`True` 不许说成「有 py 脚本」**（2026-09-21 修）：后端那一格的判据是
    `scriptKind() !== null` —— `script_source` 非空 ⇒ py、否则 `steps` 解得出非空数组 ⇒ json，
    **两种都算「有」**。所以它对一个 **json 站**也说「有」——【我量的·2026-09-21】
    `cvrefresh.com` 的榜单那一行当时写的是「配置 34（启用）· 有 py 脚本」，
    而它那份 `steps` 是一个真配置（`GET /configcheck?site=cvrefresh.com` 回的是
    「这份配置执行器都认」）。⇒ 这一格**分不出 py 与 json**，就不许替它说成 py ——
    这一屏里「脚本」＝py、「配置」＝json，两个都摆上才是那一格的实话。

    ⚠️ 把后两行合成一句「没有脚本」是本接口**专门防**的那个形状：
    「去建配置」与「去补内容」是两件不同的事，在屏幕上长成一句，运营就会做错那一步。
    """
    if value is None:
...
def rank_row_say(row: dict) -> str:
    """③ 榜单里一行的**人话**：`失败 3 次 · 配置 66（启用）· 有脚本或配置`。
