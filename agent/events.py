...
KINDS = (
    ...
    #: **人按的「关掉这个窗口」兑现了**（Task 12）。
    #: ⚠️ 与 `window_died` **不是同一件事**（不复用它，理由与 `stop_landed` 那一条同族）：
    #: `window_died` 是服务**发现**「它不在了」（人什么都没做），这一条是服务**执行了
    #: 人的请求**（他按了那个按钮）。两件事的**因**不同 —— 合成一条，读时间线的人分不出
    #: 「窗口自己到点了」与「我关的」。
    "window_closed",
    # ── 别的（目录表第 7、8、9 行 + 这一版补的）──
    "recovered",        # 服务重启过，这个 job 是从 checkpoint 捡回来的
    "human_said",       # 人说的话 / 人打过回（`who="you"`）
    #: 人按的「停」**兑现了**（Task 8 / A3）：它落在了哪（某一闸道闸上 / 这一趟停下来了）。
    #: ⚠️ 与 `human_said` **不是同一件事**（不复用它，理由与 `narration_broken` 那一条同族）：
    #: `human_said` 是**人自己那句话**（人按下去的那一刻），这一条是**服务兑现他的请求**
    #: （到闸上那一刻）。两件事发生在两个时刻、两张嘴里，合起来说就得让一张嘴同时
    #: 替人和替服务签名 —— 而「承重的不是有哪几格，是**谁填**」（契约 §二）。
    "stop_landed",
    #: 人插的那句话**真的进了它的下一轮**（Task 9）：`say` 里是「已经交给它了」。
    #: ⚠️ 与 `human_said` **不是同一件事**（不复用它，理由与 `stop_landed` 那条同族）：
    #: `human_said` 是**人自己那句话**（他按回车的那一刻），这一条是**服务把它交出去**
    #: （探路下一次问模型之前）——两个时刻、两张嘴。而它也不是 `agent_said`（那是它自己的话）。
    #: 页面那句提示就是照这个词承诺的（`console.html` 的 `MODES.steer`：
    #: 「送到之后时间线上会出现一条「交给它了」」）。
    "steer_landed",
    #: 人插的话**没送到它手上**，而这一趟**到头了**（Task 9 回归 1 / F1b + F2②）。
    #: ⚠️ 与 `steer_landed` 是一对（同一件事的两个结局），**不是**别的词换了个说法：
    #: 这一条说的是「那一轮没发出去 / 它没再问一次模型，所以**它没看到**」——
    #: 话本身没丢（还在队里，`/again` 会带上），丢的是 `/say` 答应过的那句「直达」。
    #: 谁说的：服务（`who="system"`）—— 人开的口、服务兑现不了，两件事两个时刻两张嘴
    #: （与 `stop_landed` / `steer_landed` 同族）。
    "steer_missed",
    ...
    #: **播报那条旁路自己坏了**（Task 5 修复轮 1）—— 与 `state_unreadable` 同一族：
    #: 都是一条**新**的、会静默的路，都只在这一版才被说出来。
    #: 说的是「这一趟的时间线上少东西了」：哪几遍的结果没送到，以及为什么
    #: （`selftest.run` 的护栏接住了回调抛的那个异常，报告随 state 回来之后由服务记这一条）。
    "narration_broken",
)
