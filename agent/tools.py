...
class McpSession:
    ...
    def open(cls, *, ws_url: str | None = None, host: str | None = None, port: int | None = None,
        ...
        ⚠️ **优先级（修复轮 3，口径按修复轮 4 的复审说准）**：这个会话去连谁，
        **只由调用方说的算** —— `ws_url` > `host`/`port` > 环境变量 `CDP_WS_URL`。

        **病是什么（说准）**：不是「两级顺序颠倒了」那么简单 —— `CDP_WS_URL` 这条读法
        **从这条通道出生起就没写进过这个 docstring**（写的是「两个都不给 → 交给 `cdp-mcp`
        自己按 `CDP_HOST`/`CDP_PORT` 与默认值定」，也就是说**文档说的设计里没有 Python 侧
        这次读环境**）。所以真正的病是：**一条没写进契约的读法，排在显式参数前面** ——
        一个环境变量能**悄悄改掉一条会话的去向**，而这是**唯一真连浏览器**的那条通道。

        **严重度（也说准）**：这一格**今天在本仓不可达**，不只是「口子窄」——
        `McpSession.open` 只被 `browser_agent.explore` 的两处调到，而那两处只是把
        `explore(...)` 的 `host`/`port` 转发下来，**本仓没有任何调用方传过 `host`/`port`**
        （`graph.py` 与 `service.py` 都只给 `ws_url`）。生产也不设 `CDP_WS_URL`
        （`Dockerfile` / `entrypoint.sh` 里都没有）。⇒ 是**写反了的优先级**，
        不是「今天正在出错」。

        **`ws_url` 上那个 `.strip()` 也是这一轮加的**（复审点名：「改文案的一轮里悄悄改了
        语义」）：修前 `ws_url="   "` 是**真值**，会原样发 `--ws-url '   '`；现在它算
        「没人点名」，于是 `host`/`port`（或者环境）接手。方向是「更像修、不像坏」，
        但**它是一条语义改动**，所以有一条判据钉着（`test_ws_url_that_is_only_spaces_…`）。
        """
