...
#: 4 趟被轮预算钉死的真站探路（`runtime/explore/job-*/`），每一轮推进的动作数实测
#: 0.450 / 0.500 / 0.500 / 0.600（动作 = click/form/scroll/goto）。按**最坏**那个折算：
#: `25 / 0.450 ≈ 56` 轮，再加「收尾那一轮」（模型不调工具、只说话）⇒ **57 轮**是下界。
#: 取 **80**（≈1.6 倍）吸收「走错几步 / 多观察几次」。
...
#: 上面 57 是个**结构性下界**，不是「模型自然要用多少」。什么时候再校准：
...
class Budget:
    ...
    推法（可复算）：4 趟被轮预算钉死的真站探路（`runtime/explore/job-*/`），实测每轮推进
    **0.450–0.600** 个动作；按最坏 0.450 折算，25 步 ≈ 56 轮，再加收尾那一轮
    ⇒ **57 轮的下界** ⇒ 取 **80**（≈1.6 倍）。步数取 100（> 80）：实测每轮正好一次工具调用，
    步数不许先于轮数卡死（卡死时模型连收尾那一轮都拿不到）。

    ⚠️ **「模型自然要用多少轮」仍然没量到**（量它要跑一趟真模型）—— 80 是**结构性下界**
