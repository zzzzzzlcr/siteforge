...
#: （模型不调工具、只说话）⇒ **64 轮**是下界。取 **80**（1.25 倍）吸收「走错几步 / 多观察几次」。
...
#: 上面 64 是个**结构性下界**，不是「模型自然要用多少」。什么时候再校准：
#: **放开之后的第一趟真站探路**，读 `attempts.jsonl` 的 `rounds` 那一格 ——
#: 落在新预算 80% 以上（>64）就说明还是紧的，该再放。
#: ⚠️ 那个 **>64 的门限现在恰好压在新下界上**（真·下界就是 64，不是我第一版报的 57）
#: —— 第一批样本就会响是**预期的**，别当成误报。
...
class Budget:
    ...
    **成功推进**的动作数是 **0.450 / 0.450 / 0.450 / 0.400**（只数 `result.ok == true` 的
    ...
    再加收尾那一轮 ⇒ **64 轮的下界** ⇒ 取 **80**（**1.25 倍**）。步数取 100（> 80）：
    ...
    **两个上限、一个在起作用**：实测每轮正好一次工具调用 ⇒ `max_rounds` 永远先到。
    ...
    ⚠️ 那个门限**恰好压在下界 64 上** —— 第一批样本就会响是**预期的**，不是误报。
    """
