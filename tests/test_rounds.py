...
def test_the_bail_markers_come_from_the_gates_own_code():
    ...
    ⚠️ **这条量具的射程**（复审 2026-09-18 N3 / ② 实测，与模块里 `BAILED_END_REASONS`
    那条注释**同源**）：
      ...
      | `_enter` **按名字直接叫、且定义在同一个文件里**的 helper 用下标赋值写它 | 🔴 红 |
      | helper 再叫 helper（间接） | ⚪ 看不见 |
      | helper 里用 `out.update({...})` | ⚪ 看不见 |
      | **方法调用**（`_h.hold(out)` —— `ast.Call.func` 是 `Attribute`） | ⚪ 看不见 |
      | **跨模块** helper（名字在本文件里查不到定义） | ⚪ 看不见 |
      | 拼 key / 变量当 key（连字面量都不提） | ⚪ 看不见 |

    底下一串是这个量具**做不到**的事 —— 它不假装会红（说错射程比射程不够更坏：
