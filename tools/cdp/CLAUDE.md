...
- `form`: `[selector]` (位置参数) 或 `--selector`、`--value` / `--check` / `--select` (三选一)、`--frame-id`、`--track`。内部每一次点击都走同一条落点判据，判据说的话也印在 **stderr**（只有判据有话可说时才印 —— 扣下 / 判不了 / 判成重建）。**MCP 门**上 `form` 把同一批话放进**回执**（`landing_note` / `landing_blind` / `landing`）、`click` 走 `ClickResult` 的字段 —— 只在 CLI 上说不等于说了（agent 用的是门）。扣下的三条取证：原目标还在文档里 ∧ 覆盖者不在目标子树里（ancestry，穿 shadow）∧ 不是同一个东西的另一个节点
