...
**什么时候**扣下（三条取证，缺一不扣）：
① 按下前那个节点**还在文档里**（`isConnected` —— 它只是被盖住；被重建/摘除的目标已经不在了）；
② 覆盖者**不在目标的子树里**（ancestry：走 parentNode 或 shadow host 那条链；⚠️ `<slot>` 分发**没实现**，
   那一族仍是活洞 —— 见 `internal/client.go` 的 `hitForensics` 已知限制）——「里面长出来」的浮层扣下等于把一次点击整根拿走
   （后代那一向事件照样从目标身上过；**祖先**那一向不会，两向的回执说明是分开写的）；
③ 覆盖者与原目标**不是同一个东西**（标签 + 全部属性）。
⚠️ 判据**分不出**「逐字节重建」与「一个逐字节相同的覆盖者」—— 那一格按老行为放行（宁可漏扣，也不误扣），
回执里如实这么写，不编一个说法。

⚠️ **两条路都印**：`form`（`--select` / `--check` / `--value`）内部的每一次点击走同一条判据，
所以它也在 stderr 上印同一批话；**MCP 门**上 `click` 走回执字段，`form` 的回执里带上
`landing_note` / `landing_blind` / `landing_withheld` / `landing`（判据没话可说时这几个键不出现）。

`cdp form` 的成功回执走 **stdout**（`{"ok":true,…}`，与 `click` 同一套契约；失败时不印，
错误仍旧走 stderr + 非 0 退出）—— 因为下游（产物 py 的 `_say`）**只拿得到 stdout**：
stderr 被它的 helper 丢了，而下游要的正是「这一次点击只发出了按下的那一半」这件事。

诊断的 kind 三选一：`landing-withheld`（**扣下了**）/ `landing-blind`（判据没能跑）/
`landing-passed`（判据跑了但决定**不扣**：重建 / 目标自己没了 / 里面长出来）。
⚠️ 回执里的 `landing_withheld` **只认第一个** —— 把「没扣」也算进去，那个字段就在说假话。

⚠️ `form` 那条路（`--select` / `--check` / `--value`）内部的点击**同样**会扣下抬起，
所以它也在 stderr 上印同一批话（判据没话可说时一个字都不多）——
两条路都不静默，只在一条路上说等于没说。
⚠️ **门（MCP）与 CLI 是两回事**：agent 用的是门。`cdp form` 把它印在 stderr，
而 MCP 的 `form` 把它放进**回执**（`landing_note` / `landing_blind` / `landing`）——
只在 CLI 上说，对真正的消费方等于没说。
